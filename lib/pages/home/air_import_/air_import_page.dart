...
import 'package:logistic/pages/home/air_import_/add_air_export_page.dart';
import 'package:logistic/widgets/base_scaffold.dart';

class AirImportPage extends BaseListPage<FwDocumentationViewModel> {
  ...
  const AirImportPage({
    ...
    required this.data,
    required this.token
  }) : super(title: 'Air Import', token: token);
@override
KtLogisticsToken get userToken => token;
...
void onAddPressed(BuildContext context) {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => AddAirImportPage(token: token,), // Thay AddSeaLCLImportPage bằng trang của bạn
      ),
    );
  }
