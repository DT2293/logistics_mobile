...
import 'package:logistic/pages/home/sea_lcl_Import/addsea_lcl_import.dart';
import 'package:logistic/widgets/base_scaffold.dart';

class SeaLCLImportPage extends BaseListPage<FwDocumentationViewModel> {
  final FwDocumentationViewModel data;
  final KtLogisticsToken token;

  const SeaLCLImportPage({
    super.key,
    required this.data,
    required this.token,
  }) : super(title: 'Sea LCL Import', token: token);
  ...
  void onAddPressed(BuildContext context) {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => AddSeaLCLImportPage(token: token,), // Thay AddSeaLCLImportPage bằng trang của bạn
      ),
    );
  }
  ...
  void onDeleteItem(FwDocumentationViewModel item) {
    // Xử lý xóa item (gọi API hoặc các hành động khác)
        ...
        DataCell(
          Column(
            ...
            children: item.customerConsigneeInfo != null
                ? item.customerConsigneeInfo!
                    .split(',')
                    .map((info) => Text(info.trim()))
                    .toList()
